...
func take_damage(amount: int) -> void:
	## Apply damage to enemy and update visual colour
	health -= amount
	print("Enemy took %d damage, health now: %d" % [amount, health])

	# Update colour based on new health (purple → red gradient)
	_update_color()
...
func _die() -> void:
	...
	if player_ships.size() > 0:
		...
		if ship.stats:
			...
			print("  → Score and XP awarded to player")
	else:
		push_warning("Enemy died but no player found to award score/XP")
